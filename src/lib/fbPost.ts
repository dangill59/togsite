...
//
// Announcements get a per-show flyer (date + venue overlaid on the brand
// gradient). Pre/post posts use the generic OG share card. Flyer files
// live at public/flyers/{slug}.png and are generated by gen_flyers.py
// — re-run that whenever shows.ts changes so every show has its flyer.

export type FbPostKind = 'announcement' | 'pre' | 'post';

export function imageForKind(kind: FbPostKind, slug: string): string {
  if (kind === 'announcement') {
    return `${SITE}/flyers/${slug}.png`;
  }
